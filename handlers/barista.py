...
async def cmd_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for label, items in groups:
        ...
        for t in items:
            ...
            tokens = get_tokens_for_task(t['key'])
            lines.append(f"{prefix} *{t['key']}*")
            lines.append(f"{t['name']} _(+{tokens})_")
            lines.append("")
...
async def handle_leaderboard_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for label, items in groups:
        ...
        for t in items:
            ...
            prefix = '✅' if status == 'approved' else '⏳' if status == 'pending' else '⬜'
            tokens = get_tokens_for_task(t['key'])
            lines.append(f"{prefix} `{t['key']}` — {t['name']} _(+{tokens}🪙)_")
        lines.append("")
...
async def cmd_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    recent = db.execute("""
        SELECT task_name, tokens_awarded, submitted_at FROM task_submissions
        WHERE user_id = ? AND status = 'approved'
        ORDER BY submitted_at DESC LIMIT 5
    """, (db_user['id'],)).fetchall()

    lines = [
        f"💰 *Your balance — {location['name']}*\n",
        f"🪙 Tokens: *{db_user['tokens']}*",
        f"📈 Total ever earned: {db_user['total_earned']}",
        f"🏆 Rank: *#{rank}* of {total_staff}\n",
        f"📅 Today: {today_done} tasks done",
        f"📆 This week: {week_done} tasks done\n",
    ]
    ...
    if recent:
        ...
        for r in recent:
            dt = r['submitted_at'][5:10]
            lines.append(f"  +{r['tokens_awarded']}🪙  {r['task_name']} _{dt}_")

    lines.append("\n👀 Use /leaderboard to see how you compare with the team.")
...
def _build_leaderboard(db, location, viewer_telegram_id, period):
    ...
    medals = ['🥇', '🥈', '🥉']
    lines = [f"🏆 *Leaderboard — {period_label}*\n📍 {location['name']}\n"]
    ...
    for i, row in enumerate(rows):
        ...
        lines.append(
            f"{medal} *{name}*{is_you}\n"
            f"     🪙 {row['period_tokens']} tokens · ✅ {row['period_tasks']} tasks"
        )
